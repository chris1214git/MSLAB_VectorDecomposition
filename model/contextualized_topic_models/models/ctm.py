...
class CTM:
    ...
    ###
        ...
        # Note: comment all DL revelant term and original RL
        # RL = torch.mean(torch.sum(-inputs * torch.log(word_dists + 1e-10), dim=1))
        ###
    ...
    def _predict(self, loader):
        ...
        for batch_samples in loader:
            ...
            # NDCG for word dist
            ndcg_scores = retrieval_normalized_dcg_all(word_dists, X_bow, k=self.config['topk'])
            for k, v in ndcg_scores.items():
    ...
    def get_reconstruct(self, testing_set):
        ...
        for batch_samples in loader:
            ...
            kl_loss, rl_loss, kl_loss = self._loss(X_bow, word_dists, prior_mean, prior_variance,
                              posterior_mean, posterior_variance, posterior_log_variance, recon_dists)
