...
class CTM:
    ...
    ###
        ...
        self.model = DecoderNetwork(
            ...
            dropout, learn_priors, label_size=label_size, vocab_size=len(vocab))
        ### 
    ...
    def fit(self, train_dataset, validation_dataset=None, save_dir=None, verbose=False, patience=5, delta=0,
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.validation_data is not None:
                ...
                # train epoch
                s = datetime.datetime.now()
                val_samples_processed, val_loss, val_res = self._validation(validation_loader)
                e = datetime.datetime.now()
            ...
            if epoch % 10 == 0:
                ...
                diversity = TopicDiversity(topics=self.get_topic_lists(25))
                record = open('./'+self.config['model']+'_'+self.config['target']+'.txt', 'a')
                print('---------------------------------------')
                record.write('-------------------------------------------------\n')
                print('EPOCH', epoch)
                ...
                for key,val in val_res.items():
                    ...
                    if self.config['save'] and (val >= self.config['threshold']):
                        save_model = True
                print('NPMI: ', npmi.score())
                print('Diversity: ', diversity.score())
                record.write('NPMI: '+ str(npmi.score()) + '\n')
                record.write('Diversity: '+ str(diversity.score()) + '\n')
                if save_model:
                    self.save('./')
            ###
    ...
    def _validation(self, loader):
        ...
        ### casimir
        return samples_processed, val_loss, results
        ###
    ### casimir
    # (1) New function for getting recontructed vector
    def get_reconstruct(self, testing_set):
        ...
        for batch_samples in loader:
            ...
            self.model.zero_grad()
            prior_mean, prior_variance, posterior_mean, posterior_variance, posterior_log_variance, word_dists, \
            estimated_labels, recon_dist =\
                self.model(X_bow, X_contextual, labels)

            kl_loss, rl_loss, dl_loss = self._loss(X_bow, word_dists, prior_mean, prior_variance,
                              posterior_mean, posterior_variance, posterior_log_variance, recon_dist)
