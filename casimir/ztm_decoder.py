...
if __name__ =='__main__':
    ...
    if config['model'] == 'CombinedTM':
        model = CombinedTM(bow_size=len(tp.vocab), contextual_size=contextual_size, n_components=config['topic_num'], num_epochs=config['epochs'], config=config, texts=text_for_doc2vec, vocab=tp.vocab, tp_vocab = tp.vocab, word_embeddings=word_embeddings, idx2token=dataset.idx2token)
    elif config['model'] == 'mlp':
        model = MLPDecoder(bow_size=len(tp.vocab), contextual_size=contextual_size, num_epochs=config['epochs'], config=config, texts=text_for_doc2vec, vocab=tp.vocab, tp_vocab = tp.vocab, word_embeddings=word_embeddings, idx2token=dataset.idx2token)
    else:
        model = ZeroShotTM(bow_size=len(tp.vocab), contextual_size=contextual_size, n_components=config['topic_num'], num_epochs=config['epochs'], config=config, texts=text_for_doc2vec, vocab=tp.vocab, tp_vocab = tp.vocab, word_embeddings=word_embeddings, idx2token=dataset.idx2token)
    ...
    while config['check_document']:
        ...
        # show info
        record = open('./'+config['model']+'_'+config['dataset']+'_'+config['target']+'_document.txt', 'a')
