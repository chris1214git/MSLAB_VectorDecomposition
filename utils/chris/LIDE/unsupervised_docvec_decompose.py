...
parser.add_argument('--lasso_alphal', type=float, default=0.0001)
...
class Vocabulary:
    def __init__(self, word2embedding, config):
        ...
        self.word_dim = len(word2embedding['apple'])
    def __len__(self):
...
dist = pairwise_distances(document_vectors, word_vectors,metric="cosine")
results = []

for pred, ans in tqdm(zip(dist, weight_ans)):
    res = evaluate_sklearn(-pred, ans)
    results.append(res)
...
if not args.no_sklearn:
    ...
    sk_lasso_alphal = args.lasso_alphal
    ...
    for doc_id, doc_emb in enumerate(tqdm(document_vectors[:train_size])):
        ...
        ans = weight_ans[doc_id]
        model = Lasso(positive=True, fit_intercept=False, alpha=sk_lasso_alphal, max_iter=sk_lasso_epoch, tol=0).fit(x, y)
        r2 = model.score(x, y)
...
experiment_dir = './records/dataset-{}-n_document-{}-wdist-{}-filtertopk-{}-emb-{}-dim-{}-time-{}'.format(
                                        config['dataset'],
                                        config['n_document'],
                                        config["document_vector_agg_weight"],
                                        config["topk_word_freq_threshold"],
                                        config["embedding_file"].split('/')[-1].split('.')[0],
