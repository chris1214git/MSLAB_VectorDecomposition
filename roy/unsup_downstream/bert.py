...
class BertFamily(nn.Module):
    ...
    def get_docvec(self, documents):
        ...
        if self.member == 'roberta':
            ...
            embedding = torch.mean(self.model.bert(**inputs).last_hidden_state,dim=1)
        return embedding
...
def train_model(config, data_loader, training_set, validation_set, output_dim, targets):
    ...
    optimizer_en = torch.optim.AdamW(encoder.parameters(), lr=config['lr'])
    optimizer_de = torch.optim.AdamW(decoder.parameters(), lr=config['lr'])
